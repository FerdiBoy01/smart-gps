...
const Register = () => {
    ...
    const handleRegisterSubmit = async (e) => {
        ...
        try {
            ...
            setStatus({ 
                loading: false, 
                alert: { type: 'success', message: 'Registrasi Berhasil! Mengirim OTP...' } 
            });
            ...
            setStatus({ 
                loading: false, 
                alert: { type: 'error', message: error.response?.data?.message || 'Registrasi Gagal' } 
            });
        }
    };
            ...
            <div className="w-full max-w-md bg-white shadow-xl rounded-2xl border-t-4 border-blue-600 overflow-hidden">
                <div className="p-8">
                    ...
                    <form onSubmit={handleRegisterSubmit} className="space-y-5">
                        <div className="relative group">
                            ...
                            <input name="username" type="text" onChange={handleInputChange} placeholder="Username" required
                        ...
                        <div className="relative group">
                            ...
                            <input name="email" type="email" onChange={handleInputChange} placeholder="Alamat Email" required
                                className="w-full pl-10 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-lg focus:outline-none focus:border-blue-500 focus:ring-4 focus:ring-blue-100 transition-all text-sm sm:text-base" />
                        </div>
                        ...
                        <div className="relative group">
                            ...
                            <input name="password" type="password" onChange={handleInputChange} placeholder="Password" required
                                className="w-full pl-10 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-lg focus:outline-none focus:border-blue-500 focus:ring-4 focus:ring-blue-100 transition-all text-sm sm:text-base" />
                        </div>

                        <button type="submit" disabled={status.loading}
                            className="w-full flex items-center justify-center py-3 px-4 font-bold text-white bg-blue-600 rounded-lg hover:bg-blue-700 focus:ring-4 focus:ring-blue-200 transition-all disabled:opacity-50 shadow-lg mt-6">
                ...
                <div className="bg-slate-100 p-4 text-center border-t border-slate-200">
                    <p className="text-sm text-slate-600">
                        Sudah punya akun? <Link to="/login" className="font-bold text-orange-500 hover:text-orange-600">Login disini</Link>
                    </p>
                </div>
            </div>
        </div>
